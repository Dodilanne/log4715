...
namespace UnityStandardAssets.Cameras {
  ...
  public class AutoCam : PivotBasedCameraRig {
    ...
    [SerializeField] private PlatformerCharacter2D target_character;
    ...
    protected override void FollowTarget(float deltaTime) {
      ...
      Vector3 target_position = m_Target.position;
      if (!target_character.getGrounded()) {
        target_position.y = transform.position.y;
      }

      // camera position moves towards target position:
      transform.position = Vector3.Lerp(transform.position, target_position, deltaTime * m_MoveSpeed);
